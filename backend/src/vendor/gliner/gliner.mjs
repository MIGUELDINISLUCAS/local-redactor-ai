...
import ort_CPU from 'onnxruntime-node';
const ort_WEBGPU = ort_CPU;
const ort_WEBGL = ort_CPU;
...
var SpanModel = class extends Model {
  prepareInputs(batch) {
    ...
    const createTensor = (data, shape, tensorType = "int64") => {
      const __f=data.flat(Infinity); const __a = tensorType==="int64" ? BigInt64Array.from(__f,v=>BigInt(v)) : tensorType==="bool" ? Uint8Array.from(__f,v=>v?1:0) : __f; return new this.onnxWrapper.ort.Tensor(tensorType, __a, shape);
    };
  ...
  encodeInputs(texts, promptLengths = null) {
    ...
    for (let id = 0; id < texts.length; id++) {
      ...
      tokenizedInputs.forEach((word, wordId) => {
        // SentencePiece/DeBERTa-v3 tokenizers are context-sensitive: a word gets
        // a leading metaspace (▁) marker when it follows a space. Encoding each
        // word in isolation loses that, yielding WRONG subword ids for every word
        // after the first. Prepending ▁ reproduces HF's is_split_into_words
        // tokenization exactly. Guarded by `metaspace` so non-SP models (and the
        // shipping span model) are unaffected. Special prompt tokens are atomic
        // and must be encoded as-is.
        const useMeta = this.config.metaspace && word !== '<<ENT>>' && word !== '<<SEP>>';
        let wordTokens = this.tokenizer.encode(useMeta ? '▁' + word : word).slice(1, -1);
...
var TokenDecoder = class extends BaseDecoder {
  decode(batchSize, inputLength, numEntities, texts, batchIds, batchWordsStartIdx, batchWordsEndIdx, idToClass, modelOutput, flatNer = false, threshold = 0.5, multiLabel = false) {
    // Logits layout is [batch, token, entity, position(3)] — position (start/
    // end/inside) is the INNERMOST dim in the HF token-level export. (The
    // original vendored code assumed position was outermost, which decoded
    // nothing for these models.)
    const entityStride = 3;
    const tokenStride = numEntities * 3;
    const batchStride = inputLength * numEntities * 3;
    let selectedStarts = [];
    let selectedEnds = [];
    let insideScore = [];
    ...
    modelOutput.forEach((value, id) => {
      let position = id % 3;
      let entity = Math.floor(id / entityStride) % numEntities;
      let token = Math.floor(id / tokenStride) % inputLength;
      let batch = Math.floor(id / batchStride) % batchSize;
      let prob = sigmoid(value);
...
var Gliner = class {
  ...
  async initialize() {
    ...
    const metaspace = !!this.config.metaspace;
    if (this.config.modelType == "span-level") {
      const processor = new SpanProcessor({ max_width: maxWidth, metaspace }, tokenizer, wordSplitter);
      ...
      const processor = new TokenProcessor({ max_width: maxWidth, metaspace }, tokenizer, wordSplitter);
